...
control.get("/player1/pokemon/:surnom", (req, res) => {
    ...
    connection.query("SELECT accounts.username,pokemon.id,name,surnom,nv,description,nature.natur,description FROM pokemon INNER JOIN nature ON pokemon.id_nature = nature.id INNER JOIN possede ON pokemon.id = possede.id_pokemon INNER JOIN accounts ON pokemon.id_accounts = accounts.id WHERE pokemon.surnom = ? GROUP by pokemon.id;", [req.params.surnom], function (error, results, fields) {
        ...
        if (results[0] !== undefined) {
            ...
            connection.query("SELECT statistique.namestat FROM possede INNER JOIN statistique ON possede.id_statistique = statistique.id where id_pokemon = ?;", [results[0].id], function (err, ress, fiel) {
...
control.get("/pokemon", (req, res) => {
    ...
    connection.query(`SELECT id FROM pokemon`, function (error, results, fields) {
        if (error) throw error;
...
control.get("/historique", (req, res) => {
    ...
    connection.query(`SELECT accounts.username AS "uservainqueur",id_pokemon_p1_combat AS "idpok1",id_pokemon_p2_combat AS "idpok2",date,pvrestant,id_vainqueur FROM combat INNER JOIN accounts ON combat.id_vainqueur = accounts.id GROUP by combat.id;SELECT accounts.username as "nomperdant" FROM combat INNER JOIN accounts ON combat.id_perdant = accounts.id`, function (error, results, fields) {
        if (error) throw error;
